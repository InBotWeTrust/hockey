...
Внутренний мессенджер: DM, системные каналы (`pnpm chat:seed "<name>"` + `SYSTEM_USER_ID` env), задел под чаты команд/турниров (`chats.entity_type/entity_id`). Таблицы: `chats`, `chat_members`, `messages`, `message_reactions` (миграция `004_chat.sql`). RLS нет — проверки в `chat/guards.ts` (`assertCanAccessChat`, `assertOwnsMessage`). Сервис `chat/service.ts`: `getMyChats` (LATERAL JOIN, без N+1), `findOrCreateDM` (advisory lock на пару), `getMessages` (before-cursor + батч-реакции), `sendMessage`/`deleteMessage` (soft, `content=''`)/`markChatAsRead` (lazy upsert), `searchUsers` (pg_trgm), `searchMessages` (tsvector russian), `getUnreadCounts`. REST под `/chat/*` (см. `chat/routes.ts`): list, dm, users, messages (GET/POST/DELETE), read, search, unread. Rate-limit 5 msg/sec, unread-кеш Redis 10s. Realtime — `@fastify/websocket` на `GET /chat/ws?token=<accessJWT>`, плагин `plugins/realtime.ts` (один extra ioredis sub-клиент + локальный роутер handler-ов). Сокет subscribe-ит на `chat:user:<userId>` и каждый активный `chat:system:<chatId>`. Publish из `chat/events.ts`: DM/group fan-out по `chat_members`, system — один broadcast (best-effort, swallow на ошибке). Кадры `{v:1,event:ChatEvent}`, heartbeat ping/pong 30s/10s, close 4401 на ауте/4408 на heartbeat lost. Web MVP (PR 4) — `/chat`, `/chat/new`, `/chat/:chatId` под `<PrivateRoute>`; TanStack Query (`chatKeys.{list,messages,users,unread}`), бейдж в BottomNav из `chatStore.totalUnread` через `GET /chat/unread`. PR 5 добавил `ChatSocket`: один WS на сессию через `GET /chat/ws?token=<accessJWT>`, exponential-backoff reconnect (1s → 30s), на 4401 — refresh access JWT и повторное подключение, на 4408 — реконнект; диспатч `ChatEvent` в `chatStore.applyEvent` + точечные `setQueryData`/`invalidateQueries` (`message:new` дедуп по message-id, `message:deleted` патчит `is_deleted=true, content=''`, `chat:read` инвалидирует `/chat/unread`); offline-баннер `.glass-dark` через 3s; на reconnect — refetch list + active messages. PR 6 — UI-полиш: header переехал на `arrow + avatar + title + search-toggle`, поиск разворачивается ниже по клику; reply/delete переехали из inline-иконок в long-press floating menu (`useLongPress` 500ms + `MessageActionsMenu` через portal, clamp 12px от всех краёв viewport). PR 7 — глобальный поиск: на ChatListScreen инпут с дебаунсом 300ms (`useDebouncedValue`) открывает `.glass-dark` дроп-лист (`SearchResultsDropdown`) с двумя секциями — Чаты (локально по chatHaystack) и Сообщения (через `GET /chat/search`); тап результата шлёт на `/chat/{chatId}?goto={messageId}`. Сервер `GET /chat/:chatId/messages` принимает `around=<uuid>&radius=25` (±radius сообщений вокруг анкора, asc) и `after=<iso>` (новее курсора, asc); `around` mutually exclusive с `before`/`after` (zod refine → 400), 404 на удалённый/чужой/несуществующий анкор (через `MessageNotFoundError`). `errorsPlugin` теперь маппит `ZodError → 400`. ChatRoomScreen читает `?goto`, грузит around-страницу, скроллит к bubble через `data-message-id`, добавляет 1.2s `chat-bubble--flash` и стрипает `?goto` из URL. PR 8 — реакции: миграция `005` ужесточает UNIQUE до `(message_id, user_id)` (одна реакция на юзера на сообщение); сервер `addReaction` (DELETE prev → INSERT ON CONFLICT) + `removeReaction` + `getMessageOr404` (фильтрует soft-deleted) + endpoint'ы `POST/DELETE /chat/messages/:id/reactions` с zod-enum 24-эмодзи whitelist; events `publishReactionAdded/Removed`. Web: `EMOJI_WHITELIST` (синхронизирован с сервером snapshot-тестом), `ReactionBar` под bubble (flex-wrap, своя — `.pill--dark`), `ReactionPicker` (3×8 portal) + эмодзи-полка (6 favorites + `+` SmilePlus) в `MessageActionsMenu`; `addMut`/`removeMut` через pure `switchMyReactionTo`/`removeMyReaction` с rollback; `useChatSocket.applyReactionEvent` патчит `chatKeys.messages` через `applyReactionEventToMessage` и дедупит self-events по `event.userId === meId`; удалён мёртвый `chatKeys.reactions`. `chatStore.totalUnread()` теперь считает чаты с >0 непрочитанных (а не сумму). Спек: `docs/superpowers/specs/2026-04-26-internal-chat-design.md` + `2026-04-27-chat-pr7-search-modal-design.md` + `2026-04-28-chat-pr8-reactions-design.md`.
