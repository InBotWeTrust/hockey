...
Task 4: Ruling: tournament loadout confirmation is period-boundary scoped and persists independently from profile equipment; each changed selection adjusts only its remaining per-item reservation, because the terminal-only release helper rewrites consumed accounting. Cost if wrong: a break-time selection could double-charge or erase prior consumption. The initial partial implementation was rejected until this rule and its compatibility path were covered.
Task 4: fix round 1/5 (legacy active-attempt compatibility, readiness retry, and zero-reserve terminal cleanup addressed; commits af2b3cb..f756afc).
Task 4: complete (commits 8050db5..f756afc, review clean; fresh serial fixture attempts 42/42 before review and scoped fix regressions 3/3 after review; amateur 50/50, migrations 6/6, DailyScreen 108/108, server/web TypeScript and diff checks passed).
Task 5: fix round 1/5 (fixed5 lazy pagination and technical-result rendering addressed; commits 86d070d..849fbc8).
Task 5: complete (commits 7ad8510..849fbc8, scoped re-review clean; server service 104/104, targeted web 179/179, repository typecheck, lint, and diff checks passed).
