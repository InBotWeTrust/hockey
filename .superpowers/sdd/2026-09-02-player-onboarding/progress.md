...
Task 12 approval gates: user explicitly approved the square story example and then the square real-gameplay example; the remaining 12 reference images may now be prepared locally, while dev upload/draft assembly/publication still requires separate authorization.
Task 12 local-prep Ruling: the approved gameplay frame and the original product requirement both use the courtyard, so the tutorial must use the existing `/sprites/test-court-bg-outdoor-v8.png` instead of the indoor `/sprites/new-light-court.webp`; geometry, sprites, sizes and speed overrides remain unchanged. If wrong, only the tutorial background differs, while keeping the indoor asset would visibly contradict both approvals.
Task 12 local prep: complete in the pending asset commit — exact 14-filename test performs real WebP decode and enforces 1200×1200; all 14 references pass and their contact sheet shows no unexpected embedded text; tutorial courtyard contract RED then focused 28/28 GREEN; non-Daily web 604/604 and direct DailyScreen 70/70 PASS; web typecheck/build/scoped lint/Prettier PASS. No dev upload, draft assembly, publication, push or deploy was performed.
