  ...
  it('finalizes a due participant-local day once after every timezone has closed', async () => {
    const admin = await pool.query<{ id: string }>(
      `insert into users (id, display_name, timezone, role)
       values ('00000000-0000-4000-8000-000000000801', 'Daily tournament admin', 'UTC', 'admin')
       returning id`,
    );
    const players = await pool.query<{ id: string }>(
      `insert into users (id, display_name, timezone)
       values
         ('00000000-0000-4000-8000-000000000811', 'UTC player', 'UTC'),
         ('00000000-0000-4000-8000-000000000812', 'Los Angeles player', 'America/Los_Angeles')
       returning id`,
    );
    const tournament = await pool.query<{ id: string }>(
      `insert into tournament
         (slug, title, status, regular_source, current_revision, starts_at, created_by)
       values ('daily-maintenance', 'Daily Maintenance', 'regular', 'daily_aggregate', 1,
               '2030-09-01T10:00:00.000Z', $1)
       returning id`,
      [admin.rows[0]!.id],
    );
    ...
    const first = await finalizeDueTournamentDailyDays(
      pool,
      new Date('2030-09-02T07:00:00.000Z'),
    );
    const second = await finalizeDueTournamentDailyDays(
      pool,
      new Date('2030-09-02T07:01:00.000Z'),
    );
