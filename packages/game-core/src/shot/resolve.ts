...
export function resolveShot(
  ...
  // Apply optional input-level overrides on goalie/goal frequencies. Client
  // sends them in input so server's reproduction is identical.
  const effectiveCfg = {
    ...cfg,
  ...
  const goalieState = simulateGoalie(effectiveCfg, seed, shotIndex, tGoalieCross, phaseOffsets?.goalie ?? 0);
  ...
  const goalOffsetAtGoal = simulateGoal(effectiveCfg, tGoalCross, phaseOffsets?.goal ?? 0).offsetX;
