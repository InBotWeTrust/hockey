...
function trajectory(seed: string, policy: Policy): number[] {
  ...
  while (readySceneTime + completedShots * PAUSE_MS < maxWallTime) {
    ...
    for (let index = 0; index < sampleCount; index += 1) {
      ...
      if (context.result.type !== 'goal' || context.v5Measurements === undefined) continue;
      const points = classifyMarksmanshipV5Score(context.v5Measurements).points;
    ...
    if (canonical.result.type !== 'goal' || canonical.awardedPoints !== choice.points) {
      throw new Error('calibration diverged from canonical V5 classifier');
    }
...
describe('V5 marksmanship target calibration', () => {
  it('derives ten increasing and reachable targets from thirty seeded trajectories', () => {
    ...
    expect(targets).toEqual([90, 170, 250, 340, 440, 550, 670, 790, 930, 1070]);
