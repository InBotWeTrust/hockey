...
The court uses the same amateur-game artwork and the existing player, blue amateur-goalie and goal sprites, not coloured placeholder rectangles. A vertical X-coordinate grid and readable edge labels sit above the rink artwork. In manual mode all three figures can be dragged directly or positioned numerically. One `Хитбоксы` button shows or hides the outlines for goal, goalie and player together; it never changes the result. The player's shot-line X is marked separately from the player sprite/hitbox because the former determines the shot.

The scene initially shows the selected seed's actual starting arrangement at time 0. `Назад`/`Вперёд` step buttons and a wheel/slider scrub a shared elapsed-time value, moving all three figures through their game-core trajectories using the first-period daily-game speed preset. A reproducible start selector changes the seed and thus all three phase offsets; it is not three unrelated checkboxes. `Сбросить` returns elapsed time to 0 for the currently selected start. Manual drag switches to manual positioning rather than silently altering a real seed trajectory. If a live match supplied a different speed snapshot, replay mode uses that snapshot and labels it; the default constructor uses the published first-period preset.
