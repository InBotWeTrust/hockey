...
Assert card order, card always visible, separate checked/unchecked access conditions for four combinations, direct locked URL fallback to catalog, seven sequential cards, concealed eighth card, reveal after seven completions, and repeat labels after completion.
...
At a small mobile viewport, capture the hub in locked/partial/open states, one demonstration, one practice error for each feedback class, a `6/10` failure, a `7/10` success, the second-tempo series, the concealed/revealed bonus card, reload during a run, and repeat after completion.
