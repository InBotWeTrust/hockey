...
--    Note: Postgres has no `min` aggregate over uuid, so we cannot use
--    `min(r2.id)`. Use DISTINCT ON ordered by created_at (id is the
--    deterministic tiebreaker for ties at microsecond precision).
...
```ts
  ...
  it('the same user can react to two DIFFERENT messages', async () => {
    ...
    // Scope by message_id: prior tests in this describe block also insert
    // reactions for userA, so an unscoped `where user_id = $1` would over-count.
